...
void EventWatch::flush(const char* reason) {
  ...
  if (accept_data && !logContinue()) {
    if (max_event_interval) {
      LwLog::error("Noisy %s, mec %d, mei %d", name.c_str(), max_event_count,
                   max_event_interval);
      kill_reason = NOISY;
    } else {
      LwLog::info("Max events for %s, mec %d", name.c_str(), max_event_count);
      kill_reason = MAX_EVENTS;
    }
...
void EventWatch::threadLoop() {
  ...
  pthread_mutex_lock(&mutex);
  /*End of life, generate the stats event*/
  if (LwConfig::inst()->getWatcherStats())
    notifyKill();
...
void EventWatch::notifyKill() {
  LwLog::info("%s flush stats", name.c_str());

  std::string config;
  config += "msr:" + std::to_string(max_suspend_records);
  config += ", mec:" + std::to_string(max_event_count);
  config += ", mei:" + std::to_string(max_event_interval);
  config += ", esi:" + std::to_string(event_suspend_interval);
  config += ", kl:" + std::to_string(keep_last);
  config += ", mm:" + std::to_string(mailbox_max);
...
#ifdef ANDROID_TARGET
  int lret = lct_log(LCT_EV_INFO, LwConfig::inst()->getInstanceName().c_str(),
      "MGR", 0, "Watcher killed", name.c_str(), reason, config.c_str());
  if (lret < 0)
    LwLog::error("lct_log returned %d", lret);
#else
  LwLog::info("MGR event :: Watcher killed :: %s :: %s :: %s", name.c_str(),
              reason, config.c_str());
#endif
}
