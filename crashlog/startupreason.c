...
#ifndef CONFIG_SOFIA
    i = DIM(boot_reason_array);
    while (i--) {
        if (!strncmp(bootreason, boot_reason_array[i].reason,
                     strlen(boot_reason_array[i].reason))) {
            strcpy(bootreason, boot_reason_array[i].crashtool_value);
            break;
        }
    }
#endif
...
#ifdef CONFIG_SOFIA
    // full match on WATCHDOG - only reason that generate HWWDT on SOFIA
    if (!strncmp(reason, "WATCHDOG", sizeof("WATCHDOG"))) {
        watchdog = "HWWDT_UNHANDLED";
    } else {
        /* Nothing to do */
        return 0;
    }
