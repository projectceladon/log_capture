...
int crashlog_check_mpanic_abort(){
    ...
    if (mdm_inst >= 0) {
        ...
        dateshort = get_current_time_short(1);
        snprintf(filename_tag, sizeof(filename_tag), "_%s_%s", MDMCRASH_EVNAME, dateshort);
        ...
        do {
            ...
            if (file_exists(destion)) {
                if ((cfg_collection_mode_modem() != COLLECT_BPLOG_CRASHING_ALL) ||
                    !bplogs_copied)
                    do_logs_copy(BPLOG_TYPE_OLD, mdm_inst, dir, filename_tag, MAXFILESIZE);
                remove(destion);
                bplogs_copied = 1;
            }
        } while (--mdm_inst >= 0);
