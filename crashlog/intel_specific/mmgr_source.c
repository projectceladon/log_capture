...
/*size defined here is in MB*/
#define LOGS_BP_SIZE_MAX     200
#define LOGS_BP_SIZE_DEFAULT 10
#define LOGS_AP_SIZE_MAX     20
#define LOGS_AP_SIZE_DEFAULT 5
...
struct mmgr_data {
    ...
    log_status status;                     /* optional data (mainly used for error code and to indicate the size of logs captured) */
...
static int write_mmgr_monitor_with_dbg_info(const char *chain, int fd,
    ...
    if (dbg_info->ap_logs_size < 0)
        cur_data.status.aplog_size = LOGS_AP_SIZE_DEFAULT;
    else if (dbg_info->ap_logs_size > LOGS_AP_SIZE_MAX)
        cur_data.status.aplog_size = LOGS_AP_SIZE_MAX;
    else
        cur_data.status.aplog_size = dbg_info->ap_logs_size;

    if (dbg_info->bp_logs_size < 0)
        cur_data.status.bplog_size = LOGS_BP_SIZE_DEFAULT;
    else if (dbg_info->bp_logs_size > LOGS_BP_SIZE_MAX)
        cur_data.status.bplog_size = LOGS_BP_SIZE_MAX;
    else
        cur_data.status.bplog_size = dbg_info->bp_logs_size;

    cur_data.status.type = dbg_info->type;

    if (chain) {
        //APIMR and MPANIC case
        snprintf(cur_data.string, sizeof(cur_data.string), "%s", chain);
    } else {
        snprintf(cur_data.string, sizeof(cur_data.string), "%s", dbg_info->data[0]);
        first_data = 1;
    }
    ...
    if (dbg_info->nb_data > 1) {
        ...
        for (i=0; i<cur_data.extra_nb_causes; i++) {
            if (cur_data.extra_tab_string[i]) {
               ...
               snprintf(cur_data.extra_tab_string[i], sizeof(cur_data.extra_tab_string[i]), "%s",
...
int mdm_OOS(const mdm_cli_callback_data_t * ev) {
    ...
    write_mmgr_monitor_with_extras("MOUTOFSERVICE", log_state,
                                   (struct mmgr_ct_context *)ev->context);
    return 0;
}
...
int mdm_MRESET(const mdm_cli_callback_data_t * ev) {
    ...
    write_mmgr_monitor_with_extras("MRESET", log_state,
                                   (struct mmgr_ct_context *)ev->context);
    return 0;
}
 ...
 */
static int compute_mmgr_param(char *type, e_dir_mode_t *mode, char *name) {

    struct {
        const char* pattern;
        const e_dir_mode_t mode;
        const char* type;
    } properties[] = {
        {"MODEMOFF", MODE_STATS, INFOEVENT},
        {"MSHUTDOWN", MODE_CRASH, CRASHEVENT},
        {"MOUTOFSERVICE", MODE_CRASH, CRASHEVENT},
        {"MPANIC", MODE_CRASH, CRASHEVENT},
        {"APIMR", MODE_CRASH, CRASHEVENT},
        {"MRESET", MODE_CRASH, CRASHEVENT},
        {"MSKIPCOREDUMP", MODE_STATS, INFOEVENT}
    };

    int patterns = DIM(properties);

    while (patterns--) {
        if (strstr(type, properties[patterns].pattern)) {
            *mode = properties[patterns].mode;
            sprintf(name, "%s", properties[patterns].type);
            return 0;
        }
    }

    if (!strstr(type, "START_CD" )) {
        //unknown event name
...
int mmgr_handle(unsigned int mdm_inst) {
    ...
    char filename_tag[PATHMAX];
    char type[20];
    int nbytes;
    off_t copy_aplog = 0, copy_bplog = 0;
    struct mmgr_data cur_data;
    ...
    snprintf(filename_tag, sizeof(filename_tag), "_%s_%s", type, dateshort);
    ...
    copy_aplog = (off_t)cur_data.status.aplog_size * MB;
    copy_bplog = (off_t)cur_data.status.bplog_size * MB;

    // For "TFT" event, parameters are given by the data themselves
    if (strstr(type, "TFT")) {
        switch (cur_data.status.type) {
             case DBG_TYPE_INFO:
    ...
    } else if (compute_mmgr_param(type, &event_mode, event_name) < 0) {
        return -1;
    }
    if (strstr(type, "MPANIC" )) {
        ...
        if (3 == cur_data.extra_nb_causes) {
            snprintf(data[4], sizeof(data[4]), "%s", cur_data.extra_tab_string[0]);
            if (!strcmp(cur_data.extra_tab_string[0], DUMP_STR_SUCCEED))
                snprintf(cd_path, sizeof(cd_path),"%s", cur_data.extra_tab_string[1]);
            else
                snprintf(data[5], sizeof(data[5]), "%s", cur_data.extra_tab_string[1]);
        ...
        if(cur_data.extra_nb_causes > 0) {
            for (i = 0; i<cur_data.extra_nb_causes; i++) {
                snprintf(data[i], sizeof(data[i]), "%s", cur_data.extra_tab_string[i]);
    ...
    if (copy_aplog > 0) {
        do_logs_copy(APLOG_TYPE, 0, dir, filename_tag, copy_aplog);
    }
    if (copy_bplog > 0) {
        do_logs_copy(BPLOG_TYPE, mdm_inst, dir, filename_tag, copy_bplog);
    }
