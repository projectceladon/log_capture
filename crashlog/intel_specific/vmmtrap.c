...
#ifdef CONFIG_SOFIA
static bool memorize_trap_info = FALSE;
static char *vmm_crashfolder = NULL, *vmm_key = NULL;
static int vmm_index = -1;
#define INDEX_SIZE  4
#endif
...
static void generate_event_by_type(char *type, const char *cd_path,
        const char *name, bool copy_file, bool copy_istp, int index_trap) {
    char *dir, *key, *event ;
    char des[PATHMAX];
    bool filter_event = FALSE;

    if (vmm_index >= 0 && vmm_index == index_trap) {
        //no event to create - using previous to copy istp
        dir = vmm_crashfolder;
        filter_event = TRUE;
    } else if (!crash_generated) {
        //we should generate only one crash per boot
        event = CRASHEVENT;
        key = generate_event_id(event, type);
        dir = generate_crashlog_dir(MODE_CRASH, key);
    ...
    if (filter_event) {
        LOGD("%s : Event filtered - related data copied to : %s\n", __FUNCTION__, dir);
        return;
        //pointer deallocation is supposed to be done in notify_dataready_on_vmtrap
    }

    LOGE("%-8s%-22s%-20s%s %s\n", event, key, get_current_time_long(1),
         name, (dir != NULL) ? dir : "");
    if (memorize_trap_info) {
        raise_event_dataready(key, event, type, NULL, dir, 0);
        if (vmm_crashfolder) free(vmm_crashfolder);
        if (vmm_key) free(vmm_key);
        vmm_crashfolder = dir;
        vmm_key = key;
        vmm_index = index_trap;
    } else {
        raise_event(key, event, type, NULL, dir);
        free(dir);
        free(key);
...
static int get_trap_index(const char *path, const char *name) {
    int res, result = -1;
    char index_value[PATHMAX] = {'\0',};

    //strategy depends on the name of file
    if (strstr(name, TRAP_INFO)) {
        //search Coredump_index line
        res = get_value_in_file((char *)path, "Coredump_index:", index_value, sizeof(index_value));
        if (res >=0) {
            result = atoi(index_value);
        }
    } else if (strstr(name, "coredump")) {
        //extract number from name directly
        sscanf(name, "coredump_%4d", &result);
    }
...
static void handle_dump_file(const char *path, const char *name) {
    ...
    trap_index = get_trap_index(path, name);
    ...
    if (!res) {
        ...
        if (strstr(owner, "VMM")){
            generate_event_by_type("VMMTRAP", path, name, TRUE, FALSE, trap_index);
        } else if (strstr(owner, "LINUX")) {
            get_value_in_file((char *)path, "Cause:", cause, sizeof(cause));
            if (strstr(cause, "Watchdog"))
                generate_event_by_type("SWWDT", path, name, TRUE, FALSE, trap_index);
            else
                generate_event_by_type("IPANIC", path, name, TRUE, FALSE, trap_index);
        } else if (strstr(owner, "MODEM")) {
            generate_event_by_type("MPANIC", path, name, TRUE, TRUE, trap_index);
        } else if (strstr(owner, "SECURITY")) {
            generate_event_by_type("SECPANIC", path, name, TRUE, FALSE, trap_index);
        } else {
        //unhandled if owner is not known
            generate_event_by_type("VMM_UNHANDLED", path, name, TRUE, FALSE, trap_index);
        }
    } else {
...
#ifdef CONFIG_SOFIA
static void notify_dataready_on_vmtrap() {
    char cmd[PATHMAX];
    ...
    free(vmm_crashfolder);
    free(vmm_key);
    vmm_crashfolder = NULL;
    vmm_key = NULL;
}
#endif
...
void crashlog_check_vmmtrap(bool ipanic_generated, char* watchdog) {
    ...
    if (read_trap_info(TRAP_INFO_FILE) == 1) {
        memorize_trap_info = TRUE;
        handle_dump_file(TRAP_INFO_FILE, TRAP_INFO);
        memorize_trap_info = FALSE;
    }
#endif
    if (crash_generated)
        strcpy(watchdog, "WDT");
