...
int crashlog_check_panic(char *reason, int test) {
    ...
    if (copy_to_crash) {
        ...
        // property persist.vendor.crashlogd.panic.pattern is used to fill the list of pattern
...
int crashlog_check_ram_panic(char *reason, const char *extrastring) {
    ...
    if (file_exists(LAST_KMSG)) {
        ...
        strcpy(ram_console, CONSOLE_RAMOOPS_NUM(0));
    } else {
...
int crashlog_check_kdump(char *reason __attribute__((__unused__)), int test) {
    ...
    if ((curr_stat == 3) || (test == 1)) {
        ...
        do_log_copy(crashtype, dir, dateshort, APLOG_TYPE);
