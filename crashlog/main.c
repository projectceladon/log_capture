...
#include "watchdog.h"
#include "fwcrash.h"
#include "last_vmm_log.h"
#include "intel_ecc_handler.h"
#include "check_partition.h"
...
{
    reset_logdir(STATS_DIR, 1);
    reset_logdir(LOGINFO_DIR, 1);
    reset_logdir(APLOG_DIR, 1);
    ...
    reset_log_data(LOGS_DIR, "^(aplogs|bz|crashlog|stats)[0-9]{1,3}_.*");
}
...
#ifdef CONFIG_SSRAM_CRASHLOG
    if (strstr(reason, "HWWDT_") && !check_fw_crashlog()) {
        strncpy(reason, "REBOOT", STARTUP_REASON_LENGTH);
        return;
    }
#endif
...
static void crashlog_check_early_logs(void)
{
    ...
    if (file_exists(EARLY_LOGS_FILE)) {
        copy_log(EARLY_LOGS_PATH, EARLY_LOGS_FILENAME, MATCH_EXACT,
		LOGS_DIR, "early_aplog", FALSE);
...
static void crashlog_save_boot_logs(void)
{
    ...
    /* last vmm logs */
    copy_log_simple(LAST_VMM_LOG_FILE, dir);
...
static void early_check(char *encryptstate, int test) {
    ...
    crashlog_check_vmmtrap(ipanic_generated, watchdog);
...
static void get_device_id(void) {
    ...
    if (property_get("ro.serialno", guuid, "empty_serial")>=0) {
        goto write;
    }
...
static void get_crash_env(char * boot_mode, char *crypt_state, char *encrypt_progress, char *decrypt, char *token) {
    ...
    if( property_get("vendor.crashlogd.debug.proc_path", value, NULL) > 0 )
    {
...
int process_reset_event(struct watch_entry __attribute__((unused)) *entry,
    struct inotify_event __attribute__((unused)) *event) {
    LOGE("%s: vital file removed: %s, restarting.\n", __FUNCTION__, event->name);
    property_set("vendor.crashlogd.token", CRASHLOG_ERROR_RESET);
    exit(-1);
}
...
int do_monitor() {
    ...
    set_watch_entry_callback(BUILDID_TYPE,      process_reset_event);
...
#define PROP_CRASHLOGD_ENABLE "persist.vendor.crashlogd.enable"

    char property_value[PROPERTY_VALUE_MAX];
    property_get(PROP_CRASHLOGD_ENABLE, property_value, "");

#ifdef CONFIG_RAMDUMP_CRASHLOG
    /* On Broxton-P ABL platform, force RAMDUMP_MODE in case
     * of warm reset (kernel panic or watchdog)
     */
    char startupreason[STARTUP_REASON_LENGTH] = { '\0' };

    read_startupreason(startupreason);

    if (!strcmp(startupreason, "SWWDT_RESET") ||
        !strcmp(startupreason, "WARM")) {
...
int main(int argc, char **argv) {
    ...
    if (!logsystemstate_available)
        LOGW("vendor.logsystemstate service not available\n");
    ...
    case NOMINAL_MODE :
        ...
        if (encrypt_progress[0] && !strcmp(crypt_state, "unencrypted")) {
           ...
           // early_check(encryptstate, test_flag);
            ...
            //if encrypted is in unexpected state, we should call early_check anyway
            LOGE("phone enter state: unknown.\n");
            strcpy(encryptstate,"UNKNOWN_ENCRYPTION");
            //early_check(encryptstate, test_flag);
        }
